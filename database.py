...
from config import db_username, db_password, database_name
from database_config import table_fields


class table:

    def __init__(self, name=None, logging=False, close_instantly=True, create_database=False):

        self.table_name = name
        self.logging = logging
        self.close_instantly = close_instantly

        if create_database:
            self.create_database()
        if name:
            self.mariadb_connection = mariadb.connect(user=db_username, password=db_password, database=database_name)
            self.cursor = self.mariadb_connection.cursor(buffered=True)

        # self.send_query("SET collation_connection = 'utf8_general_ci';")

    def create_database(self, logging=False):
        self.mariadb_connection = mariadb.connect(user=db_username, password=db_password)
        self.cursor = self.mariadb_connection.cursor(buffered=True)

        self.logging = logging
        self.send_query(
            f"CREATE DATABASE IF NOT EXISTS {self.brackets(database_name)} CHARACTER SET utf8 COLLATE utf8_general_ci;")
    ...
    def to_dict(self, fields, objects_data):
        oblects = []
        if fields == ["*"]:
            fields = table_fields[self.table_name]
    ...
    def select_by_max(self, fields, key_field, to_dict=False):
    ...
    def delete_table(self, sure="?"):
        ...
        if sure.lower() == "yes":
            ...
            print("Please, be careful...You tried to remove a table {table_name}".format(
                table_name=self.brackets(self.table_name)))
